...
config.read('/home/bill/Documents/energymatrix/mscS4815/oldmscL.cfg')
...
fnnames = glob.glob(data_fnbase + expname + '_B*.full')
fnnames.sort()
'''
data_fn = '/home/bill/Documents/energymatrix/realdata/Sample_mscL1/data/mscL_MG1655_seqsbins.csv'
f = open(data_fn)
...
f.close()
#roi_list = [roi_list[z] for z in range(0,len(roi_list)) if roi_list[z][0].count('N') == 0]
...
#for z in range(0,21):
    ...
    try:
        ...
        ifoot[z] =  np.sum(-pbatch*np.log2(pbatch)) + np.sum((Asum/totsum)*(condA)*np.log2(condA) + Csum/totsum*(condC)*np.log2(condC) + Gsum/totsum*(condG)*np.log2(condG) +  Tsum/totsum*condT*np.log2(condT)) - (numbins-1)*3*np.log2(np.exp(1))/(2*len(seq_mat_temp[0,0,:]))
        ifootrenorm[z] = np.sum(-pbatch*np.log2(pbatch)) + .25*np.sum((condA)*np.log2(condA) + (condC)*np.log2(condC) + (condG)*np.log2(condG) +  condT*np.log2(condT)) - (numbins-1)*3*np.log2(np.exp(1))/(2*len(seq_mat_temp[0,0,:]))
    except:
        print z
        pass
